...
class BasePolicy(ABC):
    ...
    def act(self, observations):
        ...
        ### START CODE HERE ###
        m = self.action_distribution(observations)
        sampled_actions = m.sample().cpu().numpy()
        ### END CODE HERE ###
...
class GaussianPolicy(BasePolicy, nn.Module):
    ...
    def __init__(self, network, action_dim, device):
        ...
        self.device = device
        ### START CODE HERE ###
        self.log_std = nn.Parameter(torch.zeros(action_dim, device=device, requires_grad=True))
        ### END CODE HERE ###
    ...
    def action_distribution(self, observations):
        ...
        distribution = torch.distributions.MultivariateNormal(loc=mean, covariance_matrix=covariance_matrix)
        ### END CODE HERE ###
        return distribution
